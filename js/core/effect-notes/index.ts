...
async function main() {
  const scenarios = [
    { title: "simpleLog", example: simpleLog },
    { title: "simpleComprehension", example: simpleComprehension },
    { title: "sleep", example: sleep },
    { title: "parallelMap", example: parallelMap },
    { title: "simpleTask", example: simpleTask },
    { title: "simpleBlockingAsync", example: simpleBlockingAsync },
    { title: "simpleHttpGet", example: simpleHttpGet },
    { title: "simpleHttpPost", example: simpleHttpPost },
  ];
  for (const { title, example } of scenarios) {
    printSectionTitle(title);
    await Effect.runPromise(example());
  }
}
...
const getUser = (id: number) =>
  Effect.gen(function* () {
    ...
    const res = yield* Effect.tryPromise(() => fetch(url, { headers }));
    yield* raiseForStatusDebug(res);
    const user: User = yield* Effect.tryPromise(() => res.json());
    return user;
  });
...
const createUser = (newUser: NewUser) =>
  Effect.gen(function* () {
    ...
    yield* raiseForStatusDebug(res);
    const created = yield* Effect.tryPromise(() => res.json());
    return created as NewUser;
  });

const simpleHttpPost = () =>
  Effect.gen(function* () {
