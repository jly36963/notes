...
async function basicSequentialModel() {
...
type FeatureStats = Dict<Stats>;

function getStatsForScaler(
	df: DataFrame,
...
const basicIrisExample = async () => {
	try {
		...
		const numOfInputs = X_train.listColumns().length; // num of columns
		const numOfOutputs = y_train.distinct("species").toDict().species.length; // unique output classes

		// create tensors from dataframes
		const tensors = {
			X_train: tf.tensor(X_train.toArray()),
			y_train: tf.oneHot(
				flatten(y_train.toArray() as number[][]),
				numOfOutputs,
			),
			X_test: tf.tensor(X_test.toArray()),
			y_test: tf.oneHot(flatten(y_test.toArray()) as number[][], numOfOutputs),
		};
...
const basicIrisExampleScaled = async () => {
	try {
		...
		let modelIsAlreadyTrained: boolean;
		...
		if (modelIsAlreadyTrained && shouldModelContinueTraining && epochs) {
			// train model (CONTINUATION)
			console.log("CONTINUING TRAINING OF MODEL");
			for (let i = 0; i < epochs; i++) {
				await model.fit(tensors.X_train, tensors.y_train);
		...
		// evaluate
		const evaluationResult = model.evaluate(
			tensors.X_test,
			tensors.y_test,
		) as tf.Scalar;
		...
		const predictionResult = model.predict(
			tf.tensor(newDataScaled.toArray()),
		) as tf.Tensor<tf.Rank>; // [1, 0, 0]
