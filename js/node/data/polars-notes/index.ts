import { chunk, range, round, zipObject } from "lodash-es";
...
const basicSeries = () => {
	...
	const s4 = pl.Series(
		"s4",
		range(0, 10, 1.1).map((n) => round(n, 1)),
	);
	...
	console.log("sample:", s5.sample({ frac: 0.5, seed: 1 }).toArray());
...
const basicColumnAggregation = () => {
	...
	console.log("quantile (50%): ", df.quantile(0.5));
};
...
const basicDfMutation = () => {
	...
	console.log("sample: ", df.sample({ frac: 0.5 }));
};
