...
const basicDfExport = () => {
  ...
  // df to file
  df.toText(true, ';', '/my/absolue/path/myfile.txt');
  df.toDSV(true, ';', '/my/absolue/path/myfile.txt');
  df.toCSV(true, '/my/absolue/path/myfile.csv');
  df.toTSV(true, '/my/absolue/path/myfile.tsv');
  df.toPSV(true, '/my/absolue/path/myfile.psv');
  df.toJSON(true, '/my/absolue/path/myfile.json');
}
...
const basicDfRows = () => {
  // create
  let row
  row = new Row([0, 1, 2, 3, 4]) // no columns (indices) specified
  row = new Row(
    [0, 1, 2, 3, 4], // data
