...
function printSectionTitle(s: string) {
	console.log(`\n${s.toUpperCase()}\n`);
}

function range(stop: number): Array<number>;
function range(start: number, stop: number): Array<number>;
function range(start: number, stop: number, step: number): Array<number>;
function range(a: number, b?: number, c?: number): Array<number> {
...
function round(n: number, precision = 0) {
...
const basicDfCreation = async () => {
	...
	let df = new dfd.DataFrame(records);
	...
	df = new dfd.DataFrame(colsDict);
	console.log("df from Dict<Columnlike>");
	df.ctypes.print();
	df.print();

	// From csv
	df = (await dfd.readCSV("./data/iris.csv")) as dfd.DataFrame;
...
const basicDfDetails = () => {
	...
	console.log("ctypes");
	console.log(zipObject(df.ctypes.index, df.ctypes.values as number[]));
	console.log("index");
...
const basicDfEject = () => {
	...
	console.log("df to Array<unknown>");
	console.log(output);
};
...
const basicDfSelection = async () => {
	...
	df.loc({ columns: ["sepal_length", "sepal_width"] })
		.head(5)
		.print();
	...
	df.loc({ rows: df.species.eq("Versicolor") })
		.head(5)
		.print();
...
const basicDfGrouping = async () => {
	...
	const roundNumbers = (v: any) => (typeof v === "number" ? round(v, 2) : v);
...
const basicDfMapping = () => {
	...
	df.applyMap((n: number) => n * 2).print();
};
