...
class PGDAL:
    ...
    async def execute_query(self, query: str, args: list = []) -> Union[Any, None]:
    ...
    async def get_ninja(self, id_: UUID) -> Union[Ninja, None]:
        ...
        args: List[Any] = []
        qb = (
            PostgreSQLQuery.from_(table).select("*").where(table.id == QmarkParameter())
        )
        args.append(id_)
        sql = str(qb)
        rows = await self.execute_query(sql, args)
    ...
    async def update_ninja(
        self, id_: UUID, ninja_updates: NinjaUpdates
    ) -> Union[Ninja, None]:
    ...
    async def update_jutsu(
        self, id_: UUID, jutsu_updates: JutsuUpdates
    ) -> Union[Jutsu, None]:
        """Update a ninja by id and return it"""
        table = Table(Tables.JUTSUS.value)
        args: List[Any] = []
    ...
    async def associate_ninja_and_jutsu(self, ninja_id: UUID, jutsu_id: UUID) -> None:
        ...
        table = Table(Tables.NINJAS_JUTSUS.value)
        args: List[Any] = []
        qb = (
            PostgreSQLQuery.into(table)
    ...
    async def get_ninja_with_jutsus(self, ninja_id: UUID) -> Union[Ninja, None]:
        ...
        ninjas_jutsus_table = Table(Tables.NINJAS_JUTSUS.value)
