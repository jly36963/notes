...
Revision ID: 98288922c045
Revises: 
Create Date: 2021-10-13 14:08:55.054308
...
def upgrade():
    conn = op.get_bind()
    conn.execute(sa.sql.text(upgrade_sql))


def downgrade():
    conn = op.get_bind()
    conn.execute(sa.sql.text(downgrade_sql))
