...
async def main():
    ...
    # Insert ninja
    insert_ninja_result = await mongo_dal.insert_ninja({
        '_id': str(uuid.uuid4()),
        'firstName': 'Kakashi',
        'lastName': 'Hatake',
        'age': 27
    ...
    # Select ninja
    get_ninja_result = await mongo_dal.get_ninja(ninja_id)

    print("get_ninja_result", json.dumps(get_ninja_result, indent=2))

    # Update ninja
    update_ninja_result = await mongo_dal.update_ninja(ninja_id, {
        'firstName': 'Kaka',
        'lastName': 'Sensei',
    })
    ...
    # Create jutsu
    insert_jutsu_result = await mongo_dal.insert_jutsu({
        '_id': str(uuid.uuid4()),
        'name': 'Chidori',
