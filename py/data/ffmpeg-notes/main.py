...
INPUT_DIR = os.path.join(DOWNLOADS_DIR, 'input')
OUTPUT_DIR = os.path.join(DOWNLOADS_DIR, 'output')
VID_FN = 'video.mp4'
...
def basic_trim():
    ...
    s = ffmpeg.input(input_path)
    sa = s.audio\
        .filter('atrim', **trim_config)\
        .filter('asetpts', 'PTS-STARTPTS')
    sv = s.video\
        .filter('trim', **trim_config)\
        .filter('setpts', 'PTS-STARTPTS')
    ffmpeg\
        .output(sa, sv, output_path)\
        .run(overwrite_output=True)
