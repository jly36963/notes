...
NINJAS_RECORDS: List[Ninja] = [
    {
        "id": UUID("fa6c4c93-fb64-4cd7-8b21-0e5e0f717fd6"),
        "first_name": "Kakashi",
        "last_name": "Hatake",
        "age": 27,
    },
    {
        "id": UUID("2c6c74c3-b9d6-4d49-a113-4f1a8164abe3"),
        "first_name": "Tenzo",
        "last_name": "Yamato",
        "age": 26,
    },
    {
        "id": UUID("2e9093d5-f466-40bb-be14-993276f0a497"),
        "first_name": "Iruka",
        "last_name": "Umino",
        "age": 25,
    },
    {
        "id": UUID("71547b9d-f28e-4511-b767-860bc37f148f"),
        "first_name": "Itachi",
        "last_name": "Uchiha",
        "age": 21,
    },
...
def misc_series():
    ...
    srs = pd.Series(
        np.arange(6), index=[[1, 1, 1, 2, 2, 2], ["a", "b", "c", "a", "b", "c"]]
    )  # 1a 1b 1c 2a 2b 2c
...
def misc_df():
    ...
    df3 = df2.reindex(
        columns=["col1", "col2", "col3", "col4", "col5", "col6"], fill_value=0
    )
    ...
    # find duplicates df
    df = pd.DataFrame(
        {"col1": ["a", "a", "a", "b", "b", "b"], "col2": [1, 1, 2, 1, 1, 2]}
    )  # 5 rows, 2 cols
    ...
    categories = pd.Series(
        ["int", "int", "float", "float"], index=["int1", "int2", "float1", "float2"]
    )
    ...
    cols = pd.MultiIndex.from_arrays(
        [["a", "a", "a", "b", "b"], [1, 2, 3, 1, 2]], names=["col", "sub_col"]
    )
    df = pd.DataFrame(np.arange(1, 26, 1).reshape(5, 5), columns=cols)
