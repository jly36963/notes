...
def basic_distplot():
    ...
    fig = ff.create_distplot([arr3], ["a"], bin_size=0.2, show_rug=False)
...
def basic_hist_heatmap():
    ...
    fig.add_trace(
        go.Histogram2d(x=df["x"], y=df["y"], nbinsx=20, nbinsy=20, colorscale="Blues")
    )
...
def basic_hist_contour():
    ...
    fig.add_trace(
        go.Histogram2dContour(
            x=df["x"], y=df["y"], nbinsx=20, nbinsy=20, colorscale="Blues"
        )
    )
