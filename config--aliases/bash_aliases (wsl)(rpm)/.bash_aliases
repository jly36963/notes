 mdej='conda activate mydataenv; jupyter notebook'
alias mde='conda activate mydataenv'
alias mne='conda activate mynodeenv'
alias mse='conda activate myscalaenv;'
alias msej='conda activate myscalaenv; jupyter notebook'

# file system
alias ll='ls -la'
alias home='cd ~'
alias desktop='cd "/mnt/c/Users/jly36/Desktop"'
alias back='cd -'

alias up='cd ..'
alias up2='cd ../../'
alias up3='cd ../../../'

# permissions
alias unlock='sudo chmod 777 *'
alias unlock_all='sudo chmod 777 -R *'

# update
alias qu='sudo echo "UPDATING"; sudo yum -y check-update; echo "UPGRADING"; sudo yum -y update; echo "CLEANING"; sudo yum -y clean packages'

# ----------
# motivation
# ----------

alias motivation='echo "Work hard, be considerate, be happy, be healthy, be competitive, be patient."'

