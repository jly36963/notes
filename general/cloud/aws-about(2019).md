...
- permissions
  ...
  - policies (a document that defines permissions)(assigned to uesr, group, or
    role)
...
- create role
  - you can grant permissions to a trusted entity
    ...
    - application code on an EC2 instance that needs to perform actions on AWS
      resources
    - an AWS service that needs to act on resources in your account to provide
      its features
  - example:
    - create role
    - aws service (ec2)
...
- instance types
  ...
  - spot (bid for instance capacity, great for apps with flexible start/end
    times)
...
- instance types
  ...
  - Cold HDD (SC1) (magnetic)
    - lowest cost storage for infrequently accessed workloads (cannot be boot
      volume)
...
- compute instances (EC2, Lightsail, elastic container service, lambda, batch,
  elastic beanstalk)
...
- create EC2
  ...
  - add security group (virtual firewall (kinda))
    - admin -- type (ssh), protocol (TCP), port range (22), source (anywhere
      (bad practice))
  ...
  - connect
    ...
    - in terminal -- 'chmod 400 myKeyPair.pem' (AWS will reject if too
      permissive)
...
- using Route53
  ...
  - Route 53 (Networking & content delivery)
    ...
    - traffic management
      ...
      - configure load balancer (name, scheme, IP address type, listeners,
        zones)
      - assign security group
      - configure routing
        - new target group, name, protocol (http), port (80), target type (id or
          ip address)
...
- using the CLI
  ...
  - 'unable to locate credentials'
    ...
    - create new user (ie 'dev1') (give them programmatic access and/or console
      access)
    - create group for developers (ie 'devGroup') (policy type administrator
...
- you can attach/detach roles to running EC2 instances without having to
...
- give EC2 instance permissions (using a role) to interact with an S3 bucket
  - create role
    ...
    - attach permissions (administrator access is overkill, use
      AmazonS3FullAccess)
  ...
  - apply role
    ...
    - apply role to EC2 instance (in AWS console)
      ...
      - select role from IAM role dropdown (only EC2 roles will populate
        dropdown)
...
- processing
  - OLTP -- online transaction processing (CRUD on a row) (simple, frequent
    transactions)
  - OLAP -- online analytics processing (analyze large groups of data) (complex,
    infrequent)
...
- s3 provides developers and IT teams with secure, durable, scalable object
  storage.
- s3 is easy to use, with a simple web services interface to store data.
- s3 is built for 99.99% availability (but it's guaranteed for 99.9%
  availability)
...
- after upload -- you will receive an HTTP 200 code if upload successful
  (api/CLI, not AWS console)

- Read after Write consistency -- new objects propogate quickly (puts)
- Eventual Consistency -- overwrites and deletes propogate slower (puts &
  deletes)
...
- s3 is object based -- they consist of keys/values
  ...
  - subresources -- bucket specific config (bucket policies, access control
    lists, CORS)

- storage tiers/classes (S3)
  - standard
  - standard_ia (infrequently accessed) (cheaper unless accessed frequently)
  - onezone_ia (only stored in one zone, 99.5% availability, 20% cheaper than
    s3-ia)
