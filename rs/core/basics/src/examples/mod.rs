...
/// &str -> String via `s.to_string()` or `String::from(s)`.
...
/// Returns whether or not a number is 1) prime 2) positive
fn get_is_prime_and_is_positive(n: i32) -> (bool, bool) {
...
pub fn basic_functions() -> () {
    ...
    // Return multiple
    let c = 6;
    let (prime, positive) = get_is_prime_and_is_positive(c);
    println!(
        "{} is {}, and it is {}.",
        c,
...
/// Result is a generic enum that represents Ok() or Err().
...
/// Basic lifetimes
/// Docs: https://doc.rust-lang.org/rust-by-example/scope/lifetime.html
