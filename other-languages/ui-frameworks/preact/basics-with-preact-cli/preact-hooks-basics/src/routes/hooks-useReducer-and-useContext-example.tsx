import { h, Fragment, createContext, Context } from "preact";
...
type ReactChildren = h.JSX.Element | h.JSX.Element[];
...
const TodoForm = (): h.JSX.Element => {
  ...
  const onChange = (e: InputEvent): void => {
    setFormValue((e.currentTarget as HTMLInputElement).value);
  };
  const onSubmit = (e: SubmitEvent): void => {
    e.preventDefault();
    if (!formValue.trim().length) return;
    actionAddTodo(todoDispatch, formValue);
    setFormValue("");
  };

  return (
    <form
      // @ts-expect-error: onSubmit is fucked
      onSubmit={onSubmit}
      className="todo-form"
    >
      <input
        type="text"
        value={formValue}
        // @ts-expect-error: onChange is fucked
        onChange={onChange}
      />
...
const TodoContextExample = (): h.JSX.Element => {
  return (
    <AppProvider>
      <h1>Todos</h1>
      <div>
