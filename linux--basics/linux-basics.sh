...
# / -- root
# /boot -- boot config files (used by boot loader). linux kernel. boot support files. boot config files (ie: grub.cfg)
...
# /proc -- runs processes that only exist in memory. 
    # maintains info about current state of CPU, kernel, memory, disks, partitions, file systems, running processes
    # automatically managed by system. contents are created at boot time, updated during runtime, lost at shutdown/reboot
# /lib (/usr/lib) -- library routines for many commands
# /tmp -- temporary files. (files deleted at reboot)
# /home -- user home directories
# /var -- dynamic data. where the system writes data during its operation. logs, error logs, etc.
# /run -- tmpfs. mounted during early boot process. stores volatile runtime data.
...
# /usr/local -- where system administrator installs software locally. (separate from OS, safe from OS updates.)
...
# bash scripts must begin with the following
...
# env
    # USER, PWD, HOME, SHELL, PATH, DOCKER_HOST
...
# tee
cat file.txt | tee file2.txt # tee output to stout and file
# process redirection
...
# wildcards work with most bash programs
    ...
    # ? -- any one character
    # [aeiou] -- any one character in set
    # [a-z] -- any one character in range
...
| # logical or
^abc$ # ^ (begins with) $ (ends with)

# predifined expressions
[:alnum:] # alphanumeric
[:alpha:] # letter
[:blank:] # space, tab
...
# ---------------
# use .bash_aliases (some distros won't use .bash_aliases by default)
# ---------------
