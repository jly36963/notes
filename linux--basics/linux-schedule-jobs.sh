...
# access 
    ...
    # configure access (crond) -- cron.allow & cron.deny files.
        # default -- deny files exist, allow files don't. This means users have full access.
        # cron.allow -- if this file exists, only users listed here have access.
        # cron.deny -- if no 'cron.allow' file, all users except those listed here have access.
...
# cron -- system utility which can schedule command/script to run automatically at a specified time/date
    # cron job -- scheduled task itself. 
    # crontab -- cron utility runs based on commands specified in cron table (crontab)
