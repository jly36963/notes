...
# targets -- replaced run levels (RHEL 7, Ubuntu 16.04, etc.) (systemd replaced sysVinit)
...
# linux supports 3 types of users -- 
    # root -- superuser with admin/full access. automatically created during linux installation.
    # normal -- user-level privileges. (no admin functions)
    # service -- take care of installed services. (examples: apache, ftp, mail, npt, postfix, qemu)
...
# user authentication files
    ...
    # backups of these files are also in the /etc directory (example: /etc/passwd-)
    # user auth files
        # /etc/passwd
        ...
        # /etc/shadow
            # implementatin of shadow password mechanism provides secure password for local users
                # user existence checked (/etc/passwd) and password validated (/etc/shadow)
            # passwords are encrypted and stored in this file
            # only readable by root
            # fields -- userName:encryptedPW:pwAging:minDays:maxDays:warnDays:inactiveDays:disabledDays:notUsed
                # ! -- locked password
        # /etc/group
            # group info. each line contains a group entry
            # fields -- groupName:psPlaceholder:gid:groupMembers
        # /etc/gshadow
            # shadow password implementation provides an added layer of security at the gorup level.
...
# process -- unit for provisioning system resources (ie: programs, applications, commands)
# memory -- process is created in memory, in its own address space, when a program/app/command is initiated.
# hierarchy -- processes are organized in hierarchial fashion.
# relations -- each process has a parent, and potentially children. (many attributes are passed to child processes)
# PID -- each process has a PID (process identifier). this is used by the kernel to manage/control the process.
# completion -- when completed or terminated, a process reports to its parent. resources freed, PID removed.
# boot -- several processes are started at boot. Many sit in memory, waiting for events to trigger services.
# daemons -- the above mentioned background system processes are called daemons. (critical for system operation)
# process state
    # processes change their state multiple times during their lifecycles
    # processor load, memory availability, priority, response from other apps affect process state
# states
...
# simultaneous -- a system may have hundreds or thousands of processes running simultaneously on it.
# control signals -- these can be used to alert processes of events.
    ...
    # a process will take action according to the enclosed instructions of the signal.
...
# ps -- show basic process info in 4 columns
    # PID (process id number) TTY (terminal) TIME (cumulative CPU time) CMD (name of program)
    # UID (user ID) PPID (parent process id number) C (processor utilization) STIME (start time) TIME (aggregated time)
...
# dmidecode
    ...
    # tool for dumping a computer's DMI table of contents into a human-readable format
        # hardware, BIOS, serial numbers
        # doesn't work well in virtual machines
    # 0 (BIOS) 1 (system) 2 (baseboard) 3 (chassis) 4 (processor) 5 (memory controller)
