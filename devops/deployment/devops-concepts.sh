...
# CI (continuous integration)
    # the practice of frequently merging all developer work into a main code branch.
    # CI emphasizes automated testing to ensure new merged copy still works.
    # when code change is pushed, main code branch is automatically built and tested.
...
# CD (continuous delivery)
    ...
    # IMPORTANT: app code in the main branch should be production deployable at all times
    # building blocks
        ...
        # deployment strategy that is repeatable, reliable, automated, has no down time.
...
# continuous improvement
    ...
    # metrics 
        # mean time to change -- how long does a fix take, from inception to production delivery?
        # time to measured change -- how long does a fix take, from inception to measureable change?
        # deployment frequency -- how often is code released to production?
        # deployment lead time -- how long does it take from approval of change to production deployment
...
# change in mindset
    ...
    # to -- DevOps specialists enable delivery teams to own their end-to-end CD pipelines
...
# devops teacher
    # coach -- upskill, coach, mentor delivery team on skills/competencies needed to own their CD pipeline
    # present -- participates in agile meetings, planning, retrospectives.
    # hands-on -- implements automation and tools to help delivery teams. helps the teams solve difficult problems.
    # shared -- can support multiple delivery teams, and can support more as teams mature and become autonomous
...
# AWS
    ...
    # infrastructure building blocks: servers (EC2), storage disks (EBS), networks (VPC)
    # managed services: RDS, ECS, ElasticSearch service, SQS, SES, DynamoDB, Lambda
...
# openstack
    # a cloud operating system that controls large pools of compute/storage/networking resources.
...
# docker
    # used to package applications (and deps) in a single, easily transportable container
    # isolated from host system. app containers should function identically accross different devices.
    # images -- holds a fs, can be shared by uploading into a registry
    # container -- instance of an image. contains a single process (and spawned child processes)
...
# kubernetes
    ...
    # terms
        ...
        # namespace -- an isolated 'virtual' k8s cluster running on a 'physical' cluster
...
# rancher
    ...
    # terms
        # service -- smallest deployment unit. consists of one or more containers.
            # number of containers in a single service can be scaled for performance/availability
        # stack -- group of services. can be optionally imported as a rancher-compose or docker-compose file.
        # host -- server running the rancher agent.
        # environment -- isolated unit with its own stacks/services/hosts.
            # a single rancher master can manage multiple envs running different container orchestration engines

# ECS (AWS elastic container service)
    # managed container orchestration platform
    # manages a fleet of EC2 VMs, runs containers on top of them according to specifications.
    ...
    # terms
        # tasks -- individual application components, consisting of one or more containers.
        ...
        # services -- manage tasks. 
            ...
            # optionally configures an AWS application LB for routing traffic into the tasks
...
# gitlab
    ...
    # code repos, issue tracking, wiki pages, ToDo lists, docker registry, CI/CD pipeline automation
    # gitlab CI -- pipeline automation component. supports pipeline-as-code (gitlab-ci.yml)

# rundeck
    # application for building, running, scheduling automation workflows
    # run automations locally on rundeck server (or connect to a defined set of remote hosts to run tasks on)
    # implement automation for on-demand envs, deployments, data processing, backups, etc
...
# ansible
    ...
    # terms
        # playbooks -- lists of tasks (yaml format). 
            # they represent a desired config state, or a set of steps to achieve a desired goal (ie: deployment)
...
# puppet
    ...
    # uses custom declarative language (ruby based) to describe desired state of configuration
    # often used in master/agent model, can be run standalone
    # provides resource abstraction with Facter
    # terms
        # manifests -- puppet programs defining desired state of a component in a system. 
