...
# IP
    # IP address -- numerical label assigned to each device connected to a computer network that uses IP for comm.
    ...
    # IPv4 classes
        # class a -- 1-127 first octet range, 255.0.0.0 subnet mask, 126 networks, 16m hosts,
        # class b -- 128-191 first octet range, 255.255.0.0 subnet mask, 16k networks, 65k hosts,
        # class c -- 192-223 first octet range, 255.255.255.0 subnet mask, 2m networks, 254 hosts,
    # IP details
        # network and node portions (netwwork ID + host ID) (they identify the correct destination network and node)
    # gateway -- network node that connects two networks (using different protocols) together
        # router -- common gateway.

    # bridge -- used to connect two similar types of networks.
    # static / dynamic IP
        # static -- dedicated IP, does not change
        # dynamic -- gets assigned by a DHCP server (from a pool of IP addresses, within available DHCP scope on network).
    # interfaces
        # ethernet interface -- usually the NIC (network interface card) on the back of a computer.
        # virtual interface -- exist in software only (in virtualization).
    # MAC address (media access control) -- every NIC has a hardware address (physical or virtual)
...
# TCP/UDP (reside on the transport layer of the OSI module)
    ...
    # TCP (transmission control protocol)
        ...
        # ensures that the data is received correctly and that communication is good/reliable
...
# OSI model (open systems interconnection)
    # conceptual framework that describes the functions of a networking / telecommunication system
    # model
        # 1: Physical -- conveys the bit stream across the network 
            # conveyes either electronically, mechanically, or through radio waves
        # 2: Data Link -- node-to-node data transfer (between two directly connected nodes).
            # handle error correction from the physical layer
            # two sublayers: MAC (media access control) and LLC (logical link control).
        # 3: Network -- responsible for packet forwarding. determines how data gets from source to destination.
        # 4: Transport -- coordination of the data transfer between host/destination (amount, speed, etc)
        # 5: Session -- manages connection between local and remote application. (session management)
        # 6: Presentation -- preparation/translation of application format to network format
        # 7: Application -- users interact with directly.

# NIC
    # NIC (network interface cards) -- hardware adapters, provide ports for network connectivity.
    # might be built in to motherboard or added (USB, PCIe, etc)
    # available in 1-4 port config on a single adapter
    # multiple interfaces can be configured to provide bonding/teaming for redundancy.
    # individual interfaces and bonding can both be configured with IPv4 and IPv6 assignments (in config files)
# NIC config
    # each network interface has a config file that stores IP addresses and other relevant parameters.
...
# ping
    # used to test connectivity status between source & destination over an IP network.
...
# apache -- popular, open-source, cross-platform web server
    # config files
    ...
    # setup
        # grep var/www/html /etc/httpd/conf/httpd.conf # see where html file should be
        # touch /var/www/html/index.html    
        # nano /var/www/html/index.html # add content  
    # verify status
